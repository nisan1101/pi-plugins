...
test("resuming reports every interrupted timer without starting a turn", async (t) => {
  ...
  assert.match(message.content, /timers will not resume\. Target status is unknown; the work may still be running/i);
...
test("set timer teaches the agent when and how to use it", () => {
  ...
  assert.match(tool.promptSnippet, /wake later to check asynchronous work/i);
  assert.match(tool.description, /when called alone/i);
  ...
  assert.match(guidance, /asynchronous work, including CI and deployments/i);
  assert.match(guidance, /named zmx job when available.*another process manager/i);
  assert.match(guidance, /on wake, check current status/i);
  assert.match(guidance, /delays from expected completion time/i);
  assert.match(guidance, /pending without meaningful progress.*increase successive delays/i);
  assert.match(guidance, /progress signals or completion estimates to adjust timing/i);
  assert.match(guidance, /when finished, inspect the result and continue/i);
  assert.doesNotMatch(guidance, /1–2 minutes|at least two minutes|60–120 seconds/i);
  assert.match(guidance, /call set_timer alone, after other tool calls finish/i);

  const { reason, seconds } = tool.parameters.properties;
  assert.match(seconds.description, /delay.*seconds.*delivery may be later/i);
  assert.match(reason.description, /one short line.*target and status check/i);
  assert.match(reason.description, /under 30 words.*longer identifiers or commands/i);
  assert.match(reason.description, /reuse for the same check/i);
  assert.match(reason.description, /omit background, progress history, and next-step plans/i);
  assert.doesNotMatch(guidance, /reason|30 words/i);
});
